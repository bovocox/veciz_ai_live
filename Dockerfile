...
RUN npm install ts-node typescript @types/node || echo "TypeScript bağımlılıkları yüklenemedi"

# Yedek index.js dosyası oluştur
RUN echo 'console.log("Veciz AI başlatılıyor...");' > /app/dist/index.js
RUN echo 'console.log("Ana dizin içeriği:"); console.log(require("fs").readdirSync("/app"));' >> /app/dist/index.js
RUN echo 'try { require("./src/index"); } catch (e) { console.error("Kaynak dosya yüklenirken hata:", e, "src dizini içeriği:", require("fs").readdirSync("/app/src")); }' >> /app/dist/index.js
...
# Çalıştırma komutu
CMD sh -c "nginx && cd /app && ls -la && node dist/index.js || NODE_PATH=/app/node_modules npx ts-node src/index.ts" 